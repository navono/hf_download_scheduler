...
def convert_mp3_to_wav(
    mp3_path, wav_path, target_sample_rate=16000, target_num_channels=1
):
    """
    ...
    try:
        ...
        if audio.frame_rate != target_sample_rate:
            print(
                f"正在将采样率从 {audio.frame_rate} Hz 转换为 {target_sample_rate} Hz..."
            )
        ...
        logger.info(
            f"输出 WAV 文件信息: 采样率={target_sample_rate} Hz, 声道数={target_num_channels}"
        )
    ...
    for file_name in os.listdir(input_mp3_file_dir):
        if file_name.endswith(".mp3"):
            ...
            output_wav_file = os.path.join(
                input_mp3_file_dir, file_name.replace(".mp3", ".wav")
            )
